...
try:
    ...
    _has_apex = True  # pylint: disable=invalid-name
except ImportError:
    _has_apex = False  # pylint: disable=invalid-name
...
class Trainer:
    ...
    def _rotate_checkpoints(self, use_mtime=False) -> None:
        ...
        for checkpoint in checkpoints_to_be_deleted:
            logger.info(
                "Deleting older checkpoint [%s] due to args.save_total_limit",
                checkpoint)
    ...
    def _prediction_loop(self,
            ...
            if not prediction_loss_only:
                ...
                if inputs.get("labels") is not None:
                    if label_ids is None:
                        ...
                        last_valid_indices = (
                            inputs['attention_mask'].int().sum(1) -
                            1).detach().cpu().numpy()
                    else:
                        label_ids = np.append(
                            label_ids,
        ...
        if self.compute_metrics is not None and preds is not None and label_ids is not None:
            metrics = self.compute_metrics(
                EvalPrediction(predictions=preds,
                               label_ids=label_ids,
