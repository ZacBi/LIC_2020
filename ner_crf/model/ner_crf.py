...
class BERT_BiLSTM_CRF(nn.Module):
    def __init__(self, tag_to_idx, embedding_dim, hidden_dim,
                 pretrain_model_dir, num_layers):
        """
        while use bert as feature extractor, embedding_dim should be equal to hidden_dim
        """
        ...
        self.word_embeds = BertModel.from_pretrained(pretrain_model_dir)
    ...
    def _get_lstm_features(self, sentence):
        """
        @param sentence (max_seq_len, )
        """
        self.hidden = self.init_hidden()
        embeds = self.word_embeds(sentence).view(len(sentence), 1, -1)
        # embeds (max_seq_len, 1, hidden_dim)
        lstm_out, self.hidden = self.lstm(embeds, self.hidden)
        lstm_out = lstm_out.view(len(sentence), self.hidden_dim)
        lstm_feats = self.hidden2tag(lstm_out)
        # lstm_feats (max_seq_len, tagsize)
        return lstm_feats
    ...
    def _score_sentence(self, feats, tags):
        ...
        score = torch.zeros(1)
        tags = torch.cat([
            torch.tensor([self.tag_to_idx[START_TAG]], dtype=torch.long), tags
        ])
